...
function renderVidyalayaConfig() {
  ...
  if (formSpons) formSpons.value = schoolSettings.sponsoredBy || 'Civil';
...
function renderDashboard() {
  ...
  // Priority Service Categories Breakdown (Cat 1 to Cat 5)
  ...
  document.getElementById('countCat1').innerText = cat1;
  document.getElementById('countCat2').innerText = cat2;
  document.getElementById('countCat3').innerText = cat3;
  document.getElementById('countCat4').innerText = cat4;
  document.getElementById('countCat5').innerText = cat5;

  const b1 = document.getElementById('barCat1'); if (b1) b1.style.width = total > 0 ? `${(cat1 / total) * 100}%` : '0%';
  const b2 = document.getElementById('barCat2'); if (b2) b2.style.width = total > 0 ? `${(cat2 / total) * 100}%` : '0%';
  const b3 = document.getElementById('barCat3'); if (b3) b3.style.width = total > 0 ? `${(cat3 / total) * 100}%` : '0%';
  const b4 = document.getElementById('barCat4'); if (b4) b4.style.width = total > 0 ? `${(cat4 / total) * 100}%` : '0%';
  const b5 = document.getElementById('barCat5'); if (b5) b5.style.width = total > 0 ? `${(cat5 / total) * 100}%` : '0%';
  ...
  if (stepNum === 1) {
    ...
    container.innerHTML = `
      ...
      <div id="categoryFallbackNotice" class="alert alert-info py-2 small mb-3" style="display:none;">
        ⚠️ Service Certificate rejected. Priority category will fall back from <strong>${c.priorityCat}</strong> to <strong>Cat-5 (Others)</strong>.
      </div>
...
function saveWizardStep2() {
  ...
  if (serviceStatus === 'FAIL' && currentWizardCandidate.priorityCat !== 'Cat-5') {
    currentWizardCandidate.priorityCat = 'Cat-5';
    showSamagamAlert(`Service Certificate invalid. Candidate re-classified to Cat-5 (Others).`, 'Category Fallback', 'warning');
  }
  goToWizardStep(3);
}
